@APITest
Feature: Verify CHMM GET uploaded rejection file to an organisation API works as expected
    ...
    Scenario Outline: 1. Verify that GET rejection files returns 200
     ...
     When the user send a POST request to "/identity/organisations/organisationId/rejection-files/upload" to upload "<fileNames>" file for "files" with authentication token
        ...
        When the user send a GET request to "/identity/organisations/organisationId/rejection-files" with authentication token
    ...
    Scenario Outline: 2. Verify that GET rejection files by a manufacturer user returns 403
        ...
        Given the user has authentication token for "<manufacturerId>" email id
        ...
        Examples:
            | authEmailId                       | manufacturerId                  | isOnBehalfOfGroup | isFossilFuelBoilerSeller | manufacturerUserEmailId | isResponsibleOfficer | manufacturerSROUserEmailId | creditTransferOptIn | creditTransferEmailId | fileNames          |
            | triad.test.acc.2+admin1@gmail.com | triad.test.acc.5+mfr1@gmail.com | false             | false                    | newRandom               | true                 | newRandom                  | false               | null                  | test_doc_file.docx |
