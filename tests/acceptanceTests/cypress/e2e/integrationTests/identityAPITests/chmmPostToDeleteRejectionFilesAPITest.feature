@APITest
Feature: Verify CHMM POST to delete rejection file to an organisation API works as expected
   ...
   # Scenario Outline: 1. Verify that POST delete manufacturer onboarding application by an admin user returns 204
     ...
     #   When the user send a POST request to "/identity/organisations/organisationId/rejection-files/upload" to upload "<fileNames>" file for "files" with authentication token
      ...
      #  When the user send a POST request to "/identity/organisations/organisationId/rejection-files/delete" to delete the uploaded "<fileNames>" file with authentication token
      #  Then the response status code should be 200
 ...
 # And the response body array should contain 0 items
    ...
    Scenario Outline: 2. Verify that POST delete rejection files by a manufacturer user returns 403
       ...
       When the user send a POST request to "/identity/organisations/organisationId/rejection-files/delete" to upload "<fileNames>" file for "files" with authentication token
