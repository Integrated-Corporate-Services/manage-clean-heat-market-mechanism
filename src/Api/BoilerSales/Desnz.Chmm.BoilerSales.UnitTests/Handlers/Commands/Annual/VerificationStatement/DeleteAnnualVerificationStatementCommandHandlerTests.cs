...
public class DeleteAnnualVerificationStatementCommandHandlerTests : TestClaimsBase
{
    private Mock<ILogger<DeleteAnnualVerificationStatementCommandHandler>> _mockLogger;
    private Mock<ICurrentUserService> _mockUserService;
    private Mock<IFileService> _mockFileService;
    private Mock<IOrganisationService> _mockOrganisationService;

    private readonly DeleteAnnualVerificationStatementCommandHandler _handler;

    public DeleteAnnualVerificationStatementCommandHandlerTests()
    {
        _mockLogger = new Mock<ILogger<DeleteAnnualVerificationStatementCommandHandler>>();
        ...
        _handler = new DeleteAnnualVerificationStatementCommandHandler(
            _mockLogger.Object,
    ...
    public async Task ShouldReturnAllAnnualVerificationStatementFileNames_When_FilesExist()
    {
        ...
        _mockUserService.SetupGet(x => x.CurrentUser).Returns(mockCurrentUser);
        _mockOrganisationService.Setup(x => x.GetStatus(It.IsAny<Guid>())).ReturnsAsync(getOrganisationResponse);
        _mockFileService.Setup(x => x.DeleteObjectNonVersionedBucketAsync(Buckets.AnnualVerificationStatement, $"{organisationId}/{schemeYearId}/{fileName}")).ReturnsAsync(new FileUploadResponse(null, null));
