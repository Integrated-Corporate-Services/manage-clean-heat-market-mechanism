...
public class GetApprovalFilesQueryHandlerTests
{
    private readonly GetApprovalFilesQueryHandler _handler;

    private readonly Mock<ILogger<BaseRequestHandler<GetApprovalFilesQuery, ActionResult<List<string>>>>> _logger;
    private readonly Mock<IFileService> _fileService;
    private readonly Mock<IOrganisationsRepository> _organisationsRepository;

    public GetApprovalFilesQueryHandlerTests()
    {
        _logger = new Mock<ILogger<BaseRequestHandler<GetApprovalFilesQuery, ActionResult<List<string>>>>>();
        _fileService = new Mock<IFileService>();
        _organisationsRepository = new Mock<IOrganisationsRepository>();

        _handler = new GetApprovalFilesQueryHandler(
            _logger.Object,
            _fileService.Object,
    ...
    public async Task Not_Found_When_Organisation_Not_Found()
    {
        ...
        var response = await _handler.Handle(new GetApprovalFilesQuery(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Ok()
    {
        ...
        var response = await _handler.Handle(new GetApprovalFilesQuery(organisation.Id), CancellationToken.None);
