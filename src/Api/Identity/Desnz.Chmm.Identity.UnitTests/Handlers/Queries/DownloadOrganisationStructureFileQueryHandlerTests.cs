...
{
    public class DownloadOrganisationStructureFileQueryHandlerTests
    { 
        private readonly Mock<ILogger<BaseRequestHandler<DownloadOrganisationStructureFileQuery, ActionResult<Stream>>>> _mockLogger;
        private readonly Mock<IOrganisationsRepository> _mockOrganisationsRepository;
        private readonly Mock<ICurrentUserService> _mockUserService;
        private Mock<IFileService> _mockFileService;

        private readonly DownloadOrganisationStructureFileQueryHandler _handler;

        private Guid _existingOrganisationId = Guid.NewGuid();
        private string _fileName = "file.txt";

        public DownloadOrganisationStructureFileQueryHandlerTests()
        {
            _mockLogger = new Mock<ILogger<BaseRequestHandler<DownloadOrganisationStructureFileQuery, ActionResult<Stream>>>>();
            ...
            _handler = new DownloadOrganisationStructureFileQueryHandler(
                _mockLogger.Object, 
        ...
        public async void When_UserNotLoggedIn_ReturnBadRequest()
        {
            ...
            var result = await _handler.Handle(new DownloadOrganisationStructureFileQuery(_existingOrganisationId, _fileName), CancellationToken.None);
        ...
        public async void When_OrganisationDoesNotExist_ReturnBadRequest()
        {
            ...
            var result = await _handler.Handle(new DownloadOrganisationStructureFileQuery(orgId, _fileName), CancellationToken.None);
            var actionResult = result.Result;
        ...
        public async void When_File_DoesNotExist_ReturnBadRequest()
        {
            ...
            _mockFileService
                .Setup(x => x.DownloadFileAsync(Buckets.IdentityOrganisationApprovals, $"{_existingOrganisationId}/{_fileName}"))
            .ReturnsAsync(() => fileDownloadResponse);
            var expectedResult = Responses.BadRequest(string.Format("Could not download an Organisation Structure File with name: {0} for organisation with Id {1}", _fileName, _existingOrganisationId));
