...
{
    public class DownloadRejectionCommentsFileQueryHandlerTests
    {
        private readonly Mock<ILogger<BaseRequestHandler<DownloadRejectionCommentsFileQuery, ActionResult<Stream>>>> _mockLogger;
        private readonly Mock<IOrganisationsRepository> _mockOrganisationsRepository;
        private readonly Mock<ICurrentUserService> _mockUserService;
        private readonly Mock<IFileService> _mockFileService;

        private readonly DownloadRejectionCommentsFileQueryHandler _handler;

        private Guid _existingOrganisationId = Guid.NewGuid();
        private string _fileName = "file.txt";

        public DownloadRejectionCommentsFileQueryHandlerTests()
        {
            _mockLogger = new Mock<ILogger<BaseRequestHandler<DownloadRejectionCommentsFileQuery, ActionResult<Stream>>>>();
            ...
            _handler = new DownloadRejectionCommentsFileQueryHandler(
                _mockLogger.Object, 
        ...
        public async void When_UserNotLoggedIn_ReturnBadRequest()
        {
            ...
            var result = await _handler.Handle(new DownloadRejectionCommentsFileQuery(_existingOrganisationId, _fileName), CancellationToken.None);
        ...
        public async void When_OrganisationDoesNotExist_ReturnBadRequest()
        {
            ...
            var result = await _handler.Handle(new DownloadRejectionCommentsFileQuery(orgId, _fileName), CancellationToken.None);
            var actionResult = result.Result;
        ...
        public async void When_DetailsOK_ReturnComment()
        {
            ...
            _mockFileService
                .Setup(x => x.DownloadFileAsync(Buckets.IdentityOrganisationRejections, $"{_existingOrganisationId}/{_fileName}"))
        ...
        public async void When_File_DoesNotExist_ReturnBadRequest()
        {
            ...
            var expectedResult = Responses.BadRequest(string.Format("Could not download an Rejection Comments File with name: {0} for organisation with Id {1}", _fileName, _existingOrganisationId));
