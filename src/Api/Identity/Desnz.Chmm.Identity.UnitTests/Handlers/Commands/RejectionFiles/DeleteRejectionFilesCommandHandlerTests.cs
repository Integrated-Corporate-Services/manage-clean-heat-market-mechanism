...
public class DeleteRejectionFilesCommandHandlerTests
{
    private readonly DeleteRejectionFilesCommandHandler _handler;

    private readonly Mock<ILogger<BaseRequestHandler<DeleteRejectionFilesCommand, ActionResult>>> _logger;
    private readonly Mock<IFileService> _fileService;
    private readonly Mock<IOrganisationsRepository> _organisationsRepository;

    public DeleteRejectionFilesCommandHandlerTests()
    {
        _logger = new Mock<ILogger<BaseRequestHandler<DeleteRejectionFilesCommand, ActionResult>>>();
        _fileService = new Mock<IFileService>();
        _organisationsRepository = new Mock<IOrganisationsRepository>();

        _handler = new DeleteRejectionFilesCommandHandler(
            _logger.Object,
            _fileService.Object,
    ...
    public async Task Not_Found_When_Organisation_Not_Found()
    {
        ...
        var response = await _handler.Handle(new DeleteRejectionFilesCommand(Guid.NewGuid(), "test.pdf"), CancellationToken.None);
    ...
    public async Task Bad_Request_When_File_Service_Fails()
    {
        ...
        var response = await _handler.Handle(new DeleteRejectionFilesCommand(organisation.Id, "test.pdf"), CancellationToken.None);
