...
using Desnz.Chmm.Identity.Api.Handlers.Commands.RejectionFiles;
...
namespace Desnz.Chmm.Identity.UnitTests.Handlers.Commands.RejectionFiles;

public class UploadRejectionFilesCommandHandlerTests
{
    private readonly UploadRejectionFilesCommandHandler _handler;

    private readonly Mock<ILogger<BaseRequestHandler<UploadRejectionFilesCommand, ActionResult>>> _logger;
    private readonly Mock<IFileService> _fileService;
    private readonly Mock<IOrganisationsRepository> _organisationsRepository;

    public UploadRejectionFilesCommandHandlerTests()
    {
        _logger = new Mock<ILogger<BaseRequestHandler<UploadRejectionFilesCommand, ActionResult>>>();
        _fileService = new Mock<IFileService>();
        _organisationsRepository = new Mock<IOrganisationsRepository>();

        _handler = new UploadRejectionFilesCommandHandler(
            _logger.Object,
            _fileService.Object,
    ...
    public async Task Not_Found_When_Organisation_Not_Found()
    {
        ...
        var response = await _handler.Handle(new UploadRejectionFilesCommand(Guid.NewGuid(), new List<IFormFile>()), CancellationToken.None);
    ...
    public async Task Bad_Request_When_File_Service_Fails()
    {
        ...
        var files = new List<IFormFile> { FileHelper.CreateFormFile("Rejection File 1.txt") };
        ...
        var response = await _handler.Handle(new UploadRejectionFilesCommand(organisation.Id, files), CancellationToken.None);
